    ...
    <div class="md:mx-5 flex bg-white shadow rounded border border-accent-600">
      ...
      <div class="flex-grow flex">
        ...
        >
          ...
          <!-- Content -->
          <div v-for="(item2, index) in characterList" :key="item2.id" class="relative">
            <div
            ...
            >
              ...
              ></div>
              <div v-if="item.key === 'name'" class="px-2 truncate">{{ item2.name }}</div>
              <div class="px-1 truncate" v-else>{{ item2.id }}</div>
            </div>
            <!-- Folder -->
            <div
              v-if="lineExpanded[index] && item.key === 'rank'"
            ...
            >
              <div class="p-1 rounded border border-accent-600 divide-y divide-accent-300">
                <div v-for="item3 in headerFolded" class="truncate py-1">
                  {{ item3.name + ': ' + characterList.find((item4) => item4.id === item2.id)?.image }}
                </div>
...
const lineExpanded = ref<boolean[]>(new Array(characterList.length).fill(null).map(() => false))
