  ...
  <div class="mx-1 my-3">
    ...
    <div
      ...
      <div class="flex items-end">
        <img src="https://upload.thwiki.cc/a/a6/THBWiki-LOGO-%E5%8D%9A%E4%B8%BD%E7%81%B5%E6%A2%A6%E6%96%B0%E4%BD%9C.png"
    ...
    <div class="md:mx-5 flex bg-white shadow rounded border border-accent-600">
      ...
      <div class="flex-grow flex">
        <div v-for="item in headerFixed" :key="item.key" class="font-bold border-r border-accent-600"
          ...
          <div class="py-1 px-3 whitespace-nowrap border-b border-accent-600">
            <div class="i-uil:sort-amount-down"></div>
            <div>{{ item.name }}</div>
          </div>
          <!-- Content -->
          <div v-for="(item2, index) in resultCharacter" :key="item2.rank" class="relative">
            <div class="p-1 max-w-30 md:max-w-none border-t border-accent-600 flex flex-nowrap items-center"
              :class="{ 'pb-66': lineExpanded[index] }">
              <div v-if="item.key === 'displayRank'" class="min-w-8"
                :class="lineExpanded[index] ? 'i-uil:minus' : 'i-uil:plus'"
                @click="lineExpanded[index] = !lineExpanded[index]"></div>
              <div class="px-2 truncate">{{ item2[item.key] }}</div>
            </div>
            <!-- Folder -->
            <div v-if="lineExpanded[index] && item.key === 'displayRank'"
              ...
              <div class="p-1 rounded border border-accent-600 divide-y divide-accent-300">
                ...
                <div class="py-1">角色详情：<router-link class="cursor-pointer" to="/">点击这里</router-link>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="flex flex-nowrap overflow-auto">
        <div v-for="item in headerWithoutFixed" :key="item.key" class="border-r border-accent-600">
          <!-- Header -->
          <div class="py-1 px-3 whitespace-nowrap border-b border-accent-600">{{ item.name }}</div>
          <!-- Content -->
          <div v-for="(item2, index) in resultCharacter" :key="item2.rank" class="relative">
            <div class="py-1 px-3 truncate max-w-30 md:max-w-none border-t border-accent-600"
              :class="{ 'pb-66': lineExpanded[index] }">
              {{ item2[item.key] }}
            </div>
          </div>
...
interface Header {
  name: string
  key: 'rank' | 'displayRank' | 'name' | 'voteCount' | 'firstVoteCount' | 'firstVotePercentage' | 'firstVoteCountWeighted' | 'votePercentage' | 'firstPercentage' | 'maleVoteCount' | 'malePercentagePerChar' | 'femaleVoteCount' | 'femalePercentagePerChar' | 'nameJpn' | 'characterType' | 'characterOrigin' | 'firstAppearance' | 'malePercentagePerTotal' | 'femalePercentagePerTotal'
}
const header: Header[] = [
