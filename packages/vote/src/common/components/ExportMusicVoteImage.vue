		...
		<VoteMessageBox v-model:open="exportDialogOpen" :title="`第${voteYear}回 投票卡片`" close-button>
			<div class="space-y-4 p-2 flex flex-col items-center">
				...
				<div class="w-full flex items-center justify-center gap-2 text-xs text-gray-500">
					<input id="export-use-graphql" v-model="useGraphql" type="checkbox" class="accent-blue-600" />
...
const fullMusicData = computed(() => {
	console.log('[ExportMusicVoteImage] voteMusicData:', voteMusicData.value)
	const data = voteMusicData.value.map((voteMusic: { id: string; isHonmei: boolean; reason: string }) => {
		...
		const result = {
			...
			image: getSmartUrl(fullMusic?.image || ''),
			color: getMusicColor(fullMusic?.name || voteMusic.id),
			isHonmei: voteMusic.isHonmei,
			reason: voteMusic.reason || ''
		}
		console.log(`[ExportMusicVoteImage] 曲目映射: ${result.name}, isHonmei: ${result.isHonmei}`)
		return result
	})
	console.log('[ExportMusicVoteImage] fullMusicData:', data)
	return data
})

const honmeiMusic = computed(() => {
	const honmei = fullMusicData.value.find(music => music.isHonmei)
	console.log('[ExportMusicVoteImage] honmeiMusic:', honmei)
	return honmei
})

const otherMusics = computed(() => {
	const others = fullMusicData.value.filter(music => !music.isHonmei)
	console.log('[ExportMusicVoteImage] otherMusics:', others)
	return others
})
...
async function resolveVoteMusicData() {
	...
	try {
		const { data, error, usedMode } = await getExportMusicDataFromDataSource(mode)
...
function downloadImage() {
	...
	link.download = `th-music-vote-${Date.now()}.png`
	link.click()
}

async function shareImage() {
	if (!imageBlob.value) return
	const file = new File([imageBlob.value], 'music-vote-card.png', { type: 'image/png' })
	try {
