// Static display config keyed by questionnaire.key from backend
export const questionnaireDisplayByKey: Record<string, { name: string; desc: string; image: string }> = {
...
export const categoryDisplay: Record<string, { name: string; desc: string }> = {
  main: { name: '主要问卷', desc: '您至少需要选择填写两个主要问卷' },
  extra: { name: '额外问卷', desc: '您至少需要选择填写一个额外问卷' },
}
